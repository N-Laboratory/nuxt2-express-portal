...
import Check from '~/pages/resetPassword/check.vue'
...
let wrapper: Wrapper<Check & { [key: string]: any }>
...
describe('次へ押下時の動作確認', () => {
  test('アカウントが存在しない場合は、エラーメッセージ(SweetAlert)が表示されること', async () => {
    // Arrange
    initializeAxios({
      $post: jest.fn().mockResolvedValue(0),
  ...
  test('アカウントが存在する場合は、Storeにユーザー情報を登録して次画面へ遷移すること', async () => {
    // Arrange
    initializeAxios({
      $post: jest.fn().mockResolvedValue(1),
    } as unknown as NuxtAxiosInstance)
    ...
    expect(wrapper.vm.$data.user.id).toBe(1)
    expect(mutations.updateUser).toHaveBeenCalled()
    expect(router.push).toBeCalledWith('input')
  })
